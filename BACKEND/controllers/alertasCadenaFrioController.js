...
const crearAlerta = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, mensaje: "Error al registrar la Alerta" });
    }
};
...
const cambiarEstadoAlerta = async (req, res) => {
    try {
        ...
        const [alertaExistente] = await db.query('SELECT * FROM Alertas_Cadena_Frio WHERE id_alerta = ?', [id]);
        ...
        if (!estadosValidos.includes(estado)) {
            return res.status(400).json({ success: false, mensaje: `Estado inválido. Debe ser uno de: ${estadosValidos.join(', ')}.` });
        }
        ...
        res.status(500).json({ success: false, mensaje: "Error al cambiar el estado de la Alerta", error: error.message });
    }
};
