    ...
    try {
        ...
        const [inventario] = await db.query(`
            SELECT 
                ...
                IS.cantidad_actual, 
                IS.fecha_ultima_actualizacion
            FROM Inventario_Stock IS
            JOIN Lotes L ON IS.id_lote = L.id_lote
            JOIN Vacunas V ON L.id_vacuna = V.id_vacuna
            JOIN Ubicaciones U ON IS.id_ubicacion = U.id_ubicacion
        `);
...
const crearInventarioStock = async (req, res) => {
    try {
        ...
        // Validación básica
        if (!id_lote || !id_ubicacion || cantidad_actual === undefined || cantidad_actual <= 0) {
            return res.status(400).json({
                success: false,
                mensaje: "Lote, ubicación y una cantidad positiva son obligatorios para el ingreso de stock."
            });
        }
        ...
        if (stockExistente.length > 0) {
            return res.status(409).json({ 
                success: false,
                mensaje: "Ya existe un registro de stock para este lote en esta ubicación. Por favor, use la función de 'actualizar' para incrementar la cantidad."
            });
        }
...
const actualizarInventarioStock = async (req, res) => {
    try {
        ...
        const [stockExistente] = await db.query('SELECT * FROM Inventario_Stock WHERE id_inventario = ?', [id]);
        if (stockExistente.length === 0) {
        ...
        if (cantidad_actual === undefined || isNaN(cantidad_actual) || cantidad_actual < 0) {
            return res.status(400).json({
                success: false,
        ...
        if (cantidad_actual > stockExistente[0].cantidad_actual && stockExistente[0].cantidad_actual > 0) {
        ...
        if (resultado.affectedRows === 0) {
             return res.status(404).json({
                success: false,
                mensaje: "Error al actualizar la cantidad de stock. Ningún registro afectado."
            });
        }
